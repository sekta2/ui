...
function PANEL:SUI_GetMinimumSize()
    ...
    if count > 1 then
        total_w = total_w + separation * (count - 1)
    end
...
function PANEL:PerformLayout(width, height)
    ...
    local remaining = math.max(width - total_min, 0)

    local x = 0
    for i, child in ipairs(children) do
        local min_w, min_h = min_sizes[i].w, min_sizes[i].h
        local allocated_w = min_w

        if HasFlag(child.container_size_horizontal, SektaUI.SIZE_EXPAND) and expand_ratio_sum > 0 then
            local ratio = child.container_stretch_ratio or 1
            allocated_w = allocated_w + remaining * (ratio / expand_ratio_sum)
        end

        local offset_x, actual_w = ResolveAxis(child.container_size_horizontal, allocated_w, min_w)
        local offset_y, actual_h = ResolveAxis(child.container_size_vertical, height, min_h)

        child:SetPos(x + offset_x, offset_y)
        child:SetSize(actual_w, actual_h)

        x = x + allocated_w + separation
    end
end
...
local DermaStyle = SektaUI.Default.Themes.Derma:AddElement("SUI_HBoxContainer")
...
vgui.Register("SUI_HBoxContainer", PANEL, "SUI_Control")
